...
public class PMTLGrouperTest_singleElementProductCut extends PMTLGrouperTest {

	/**
	 * Tests what happens if there were was a single element 
	 * passed in with MTLs equal to the relevantPMTLs, 
	 * but with a single episode rather than the season (forcing a cut)
	 */
	@Test
	public void singleElementProductCut() {
		...
		//PMTLs that contain only the relevantPMTLs
		{
			...
			//Other episodes - can safely check for the exact groups b/c only 1 dimension got cut
			{
				...
				productIds.remove(Seinfeld_SEASON_1_EPISODE_01.getProductId().intValue());
