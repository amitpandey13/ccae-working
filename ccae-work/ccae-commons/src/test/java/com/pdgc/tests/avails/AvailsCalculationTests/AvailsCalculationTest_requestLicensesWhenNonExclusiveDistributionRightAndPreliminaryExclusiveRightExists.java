...
public class AvailsCalculationTest_requestLicensesWhenNonExclusiveDistributionRightAndPreliminaryExclusiveRightExists
		extends AvailsCalculationTest {
	/*
	 * Request Licenses When Non Exclusive Distribution Right And Preliminary
	 * Exclusive Right Exists
	 *
	 */
	@Test
	public void requestLicensesWhenNonExclusiveDistributionRightAndPreliminaryExclusiveRightExists()
		...
		rightsIn.setRightType(TestRightType.DISTRIBUTION_RIGHTS_NONEXCLUSIVE);

		TestDealStrand preliminaryNonExclusive = new TestDealStrand(simpleEpisodeLicense);
		preliminaryNonExclusive.setTerm(new Term(DateTimeUtil.createDate(2020, 1, 1), DateTimeUtil.createDate(2021, 12, 31)));
		preliminaryNonExclusive.setOrigTerm(new Term(DateTimeUtil.createDate(2020, 1, 1), DateTimeUtil.createDate(2021, 12, 31)));
		preliminaryNonExclusive.setRightType(TestRightType.PRELIM_EXCLUSIVE_LICENSE);
		...
		AvailsQuery availsCriteria = mock(AvailsQuery.class);
        {
            ...
            when(availsCriteria.getEvaluatedPrimaryTerm()).thenReturn(new Term(
                ...
                DateTimeUtil.createDate(2022, 12, 31)));
		...
		for (Set<LeafPMTLIdSet> pmtlGroup : pmtlIdSets) {
			// term: year 2020-2021 has exclusive right in
			{
                ...
                validatePMTLTR(Availability.CONDITIONAL_DEAL, availsCalcResult, pmtlGroup, nonExclusiveLicenseRequest, term, timePeriod);
