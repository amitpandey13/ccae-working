...
public class AvailsCalculationTest_nearestMatchSingleLevelCorpOnlyMultipleStrandsWithCalcOrder1
	 ...
	 */
	@Test
	public void nearestMatchSingleLevelCorpOnlyMultipleStrandsWithCalcOrder1()
			throws FileNotFoundException {
		...
		rightsIn.setActualPMTL(new PMTL(rightsIn.getPMTL().getProduct(), basc, comoros, english));
		rightsIn.setPMTL(new PMTL(rightsIn.getPMTL().getProduct(), basc, comoros, english));
		Term rightsInTerm = new Term(DateTimeUtil.createDate(2017, 1, 1), Constants.PERPETUITY);
		...
		pmtlIdSets = getMappedPMTLIdSets(
			...
			Seinfeld_SEASON_1_EPISODE_01, basc,
			mayotte, 
			english
		);
