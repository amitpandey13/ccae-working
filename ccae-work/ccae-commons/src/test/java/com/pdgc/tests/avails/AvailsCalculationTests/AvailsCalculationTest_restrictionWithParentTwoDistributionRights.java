...
public class AvailsCalculationTest_restrictionWithParentTwoDistributionRights extends AvailsCalculationTest {
	/* 
	 * A distribution right has a restriction tied to it. 
	 * A second distribution right has the same PMTL as the restriction. 
	 * The restriction and second distribution right negate each other and return available. 
	 */
	@Test
	public void restrictionWithParentTwoDistributionRightsTest() throws FileNotFoundException {
		...
		rightsIn.setRightStrandId(100L);
		...
		restriction.setParentRightSourceId(rightsIn.getRightSource().getSourceId());
		
		TestDistributionStrand newRightsIn = new TestDistributionStrand(simpleEpisodeDistributionRights);
		TestRightSource corpSource2 = new TestCorpSource(simpleEpisodeDistributionRights.getRightSource().getSourceType(), simpleEpisodeDistributionRights.getRightSource().getSourceId());
		corpSource2.setSourceId(corpSource2.getSourceId() + 1);
		newRightsIn.setTerm(new Term(DateTimeUtil.createDate(2017, 1, 1), DateTimeUtil.createDate(2017, 12, 31)));
		newRightsIn.setOrigTerm(new Term(DateTimeUtil.createDate(2017, 1, 1), DateTimeUtil.createDate(2017, 12, 31)));
		newRightsIn.setTerrLang(new TerrLang(usa, english));
		...
		for (Set<LeafPMTLIdSet> pmtlGroup : pmtlIdSets) {
			...
			// term: year 2017 - has the restriction
			{
