...
public class AvailsCalculationTest_expiredRightsRestrictionWithExpiredPeriodEndingAtPerpetuity extends AvailsCalculationTest {
	
    /*
     * Scenario 1 with expired rights expired rights ending in perpetuity
     *
     *
     */
	@Test
	public void expiredRightsRestrictionWithExpiredPeriodEndingAtPerpetuity() throws FileNotFoundException {
		...
		expiredRights.setTerm(new Term(DateTimeUtil.createDate(2022, 1, 1), Constants.PERPETUITY));
		expiredRights.setTerm(new Term(DateTimeUtil.createDate(2022, 1, 1), Constants.PERPETUITY));
		expiredRights.setRightSource(crs12347L);
		expiredRights.setRightType(TestRightType.EXPIRED_RIGHTS_RESTRICTION);

		List<RightStrand> rightStrands = Arrays.asList(rightsIn, expiredRights);
		...
		for (Set<LeafPMTLIdSet> pmtlGroup : pmtlIdSets) {
			...
			// term: year 2022-perpetuity rights have expired
			{
				term = new Term(DateTimeUtil.createDate(2022, 1, 1), Constants.PERPETUITY);
