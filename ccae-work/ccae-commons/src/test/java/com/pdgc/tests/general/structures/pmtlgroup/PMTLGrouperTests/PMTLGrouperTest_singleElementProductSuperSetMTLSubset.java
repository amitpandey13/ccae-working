...
public class PMTLGrouperTest_singleElementProductSuperSetMTLSubset extends PMTLGrouperTest {
	
	/**
	 * Tests what happens if there were was a single element passed in with where the product is a superset
	 * and where the media, territory, and languages were subsets of the relevant PMTLS
	 */
	@Test
	public void singleElementPMTLSuperSetNoCutTest() {
		...
		Set<LeafPMTLIdSet> leafPMTLIdSets = new HashSet<LeafPMTLIdSet>(); 
		for (IdSetGroup<LeafPMTLIdSet> idSetGroup : pmtlGroups) {
			leafPMTLIdSets.addAll(idSetGroup.getIdSets()); 
		}
		assertEquals(4, leafPMTLIdSets.size());
		...
		//PMTL with the object - intersected with the relevant pmtl filters
		{
			...
			//Check the actual pmtls of the group
			{
		...
		//PMTLs that contain only the relevantPMTLs
		{
			...
			assertFalse(matchingGroups.isEmpty());
			assertEquals(3, matchingGroups.size());
