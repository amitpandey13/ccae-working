...
public class AvailsCalculationTest_multiParentProductSingleSeasonLicense extends AvailsCalculationTest {
	/*
	 ...
	 * Season 1+2 contains 101, 102, 201, 202 License is taken out on Broadcast
	 * Season 1. When searching for availability on Sales Season 1+2, 101 and 102
	 * are unavailable and 201 and 202 are available.
	 * 
	 * This unit test DOES NOT prove the Sales Season 1+2 unavailability, as
	 * AvailsCalculationTest only provides availability at the episodic leaf level.
	 * However, AvailsRollupResult provides the ultimate unavailability for Sales
	 * Season 1+2.
	 */
	@Test
	public void multiParentProductSingleSeasonLicenseTest() {
		...
		TestDealStrand licenseSeason1 = new TestDealStrand(				
			2L, 
			new PMTL(Seinfeld_SEASON_1, USEnglishPTV),
			new TermPeriod(
		...
		List<RightStrand> rightStrands = Arrays.asList(rightsIn, licenseSeason1);
		
		HierarchyMapEditor<Product> productHierarchy = new HierarchyMapEditor<>(AvailsCalculationTest.productHierarchy);
		{
        ...
        AvailsCalculationResult availsCalcResult = runAvails(
            runParams,
            rightStrands, 
            Arrays.asList(Seinfeld_SEASON_1_AND_2),
